...
CREATE TABLE hr.regiao (
                id_regiao INTEGER NOT NULL,
                nome VARCHAR(25) NOT NULL,
                CONSTRAINT regiao_pk PRIMARY KEY (id_regiao)
);
COMMENT ON TABLE hr.regiao IS 'armazena os dados relativas as regiões do globo terrestre no qual a entidade está distribuída.';
COMMENT ON COLUMN hr.regiao.id_regiao IS 'Chave primária da tabela região,';
COMMENT ON COLUMN hr.regiao.nome IS 'Nome completo da região';
...
COMMENT ON TABLE hr.pais IS 'armazena as informações relativas aos países nos quais a empresa está alocada.';
COMMENT ON COLUMN hr.pais.id_pais IS 'Chave primária da tabela países, usar apenas a abreviatura do país e não seu nome por extenso.';
COMMENT ON COLUMN hr.pais.nome_pais IS 'armazena o nome completo do país, por extenso.';
COMMENT ON COLUMN hr.pais.id_regiao IS 'Chave estrangeira proveniente da tabela regiões';
...
CREATE TABLE hr.localizacoes (
                ...
                uf VARCHAR(25),
                id_pais CHAR(2),
                CONSTRAINT loc_pk PRIMARY KEY (id_localizacao)
);
COMMENT ON TABLE hr.localizacoes IS 'Armazena as localizações dos escritórios da empresa.
Não armazena endereços de clientes.';
COMMENT ON COLUMN hr.localizacoes.id_localizacao IS 'Chave primária da tabela, armazena o identificador único da tabela localizacao.';
COMMENT ON COLUMN hr.localizacoes.endereco IS 'Armazena o endereço de cada escritório';
COMMENT ON COLUMN hr.localizacoes.cep IS 'Armaze o código postal de cada localização.';
COMMENT ON COLUMN hr.localizacoes.cidade IS 'cidade onde está localizado o escritório da entidade, ou demais localizações.';
COMMENT ON COLUMN hr.localizacoes.uf IS 'Estado abreviado ou por extenso de cada localização.';
...
COMMENT ON TABLE hr.cargos IS 'Tabela que contém os cargos existentes e a respectiva faixa salarial.';
COMMENT ON COLUMN hr.cargos.id_cargo IS 'Chave primária da tabela, identificador único de cada cargo';
COMMENT ON COLUMN hr.cargos.cargo IS 'Nome do cargo, tal qual estrutura hierárquica do órgão/entidade.';
COMMENT ON COLUMN hr.cargos.salario_minimo IS 'O menor salario admitido para um cargo.';
...
COMMENT ON TABLE hr.empregados IS 'Esta tabela armazenará informações acerca dos empregados do órgão/entidade.';
COMMENT ON COLUMN hr.empregados.id_empregado IS 'Chave única de identificação de cada empregado.';
COMMENT ON COLUMN hr.empregados.nome IS 'Nome completo de cada empregado';
COMMENT ON COLUMN hr.empregados.email IS 'Parte inicial do e-mail, antes do @.';
COMMENT ON COLUMN hr.empregados.telefone IS 'telefone do empregado - com identificador do país e DDD.';
COMMENT ON COLUMN hr.empregados.data_contratacao IS 'Data em que o funcionário iniciou no cargo atual.';
COMMENT ON COLUMN hr.empregados.id_cargo IS 'Chave estrangeira da tabela cargos, referenciando o id do cargo de cada empregado.';
COMMENT ON COLUMN hr.empregados.salario IS 'Salário mensal atual do empregado.';
COMMENT ON COLUMN hr.empregados.comissao IS 'Percentual de comissão (exclusivo para funcionários do departamento de vendas).';
COMMENT ON COLUMN hr.empregados.id_departamento IS 'Chave estrangeira para tabela departamento.';
COMMENT ON COLUMN hr.empregados.id_supervisor IS 'Autorrelacionamento, haja vista que todo supervisor precisa primeiro ser um empregado.';
...
CREATE TABLE hr.departamentos (
                ...
                id_gerente INTEGER,
                CONSTRAINT id_departmento_pk PRIMARY KEY (id_departamento)
);
COMMENT ON TABLE hr.departamentos IS 'Esta Tabela armazena informações acerca de todos os departamento da entidade.';
COMMENT ON COLUMN hr.departamentos.id_departamento IS 'Identificador único de cada departamento.';
COMMENT ON COLUMN hr.departamentos.nome IS 'Nome de cada departamento, tal qual estrutura hierárquica de cada órgão/entidade.';
COMMENT ON COLUMN hr.departamentos.id_localizacao IS 'Chave estrangeira da tabela localizações, referenciando em qual local cada departamento está alocado.';
COMMENT ON COLUMN hr.departamentos.id_gerente IS 'Chave estrangeira para identificação do gerente.';
...
CREATE TABLE hr.historico_cargos (
                ...
                id_departamento INTEGER,
                CONSTRAINT hist_cargos_pk PRIMARY KEY (id_empregado, data_inicial)
);
COMMENT ON TABLE hr.historico_cargos IS 'Tabela que armazena toda a movimentação dos empregados, promoções, demissões e afins.';
COMMENT ON COLUMN hr.historico_cargos.id_empregado IS 'Chave primária particionada, que também é uma chave estrangeira.';
COMMENT ON COLUMN hr.historico_cargos.data_inicial IS 'Chave primária particionada da tabela, armazena a data de admissão ou movimentação funcional.';
COMMENT ON COLUMN hr.historico_cargos.data_final IS 'Último dia do empregado naquele cargo, necessita ser maior do que a data inicial.';
COMMENT ON COLUMN hr.historico_cargos.id_cargos IS 'Chave estrangeira da tabela cargos, corresponde aos cargos que cada funcionário ocupou no passado.';
COMMENT ON COLUMN hr.historico_cargos.id_departamento IS 'Chave estrangeira da tabela departamentos.';

ALTER TABLE hr.pais ADD CONSTRAINT regiao_pais_fk
FOREIGN KEY (id_regiao)
REFERENCES hr.regiao (id_regiao)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;

ALTER TABLE hr.localizacoes ADD CONSTRAINT pais_localizacoes_fk
FOREIGN KEY (id_pais)
REFERENCES hr.pais (id_pais)
ON DELETE NO ACTION
ON UPDATE NO ACTION
NOT DEFERRABLE;
...
INSERT INTO hr.regiao (id_regiao, nome) values (4,'Middle East and Africa');

INSERT INTO hr.pais (id_pais, nome_pais, id_regiao) VALUES
('AR','Argentina','2');
INSERT INTO hr.pais (id_pais, nome_pais, id_regiao) VALUES
('AU','Australia','3');
INSERT INTO hr.pais (id_pais, nome_pais, id_regiao) VALUES
...
INSERT INTO hr.localizacoes (id_localizacao, endereco, cep,
